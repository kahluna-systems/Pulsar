...
- **NEXT UP — host visibility & test vantage (agreed + designed 2026-07-06, not yet built):**
  - **Phase A (small)**: add `psutil`; `GET /api/system` (engineer-gated): CPU/mem/disk/load/uptime + per-NIC name/state/MAC/MTU/speed/IPs/RX-TX counters (+ default gw via `/proc/net/route` or add `iproute2`). Host networking means the container sees the REAL NICs. UI: new System tab (host cards + NIC table), small health strip on Dashboard, and populate the packet-capture interface dropdown from this API (closes the hardcoded-list item below). This endpoint is deliberately shaped to become the hub-mode heartbeat payload (registry `metadata_json`) later.
  - **Phase B (medium)**: per-run vantage — "Test from: [auto ▾]" selector in the toolbar next to the attribution bar (auto = kernel routing = today's behavior; session-sticky). Plumb source into every outbound runner: `ping -I`, `mtr -I`/`-a`, `traceroute -i`/`-s`, `iperf3 -B`, `dig -b`/dnspython `source=`, Python-socket `bind()` for TCP/SSL. Stamp the chosen source into test config so history/reports show "measured via ensX". Portal tests always run auto. iperf3 server mode gains an optional listen-address bind (the inbound-direction equivalent). Caveats: address-bind requires routing to permit egress; device-bind (`-I`, SO_BINDTODEVICE) needs cap_net_raw — our capped binaries have it; unprivileged Python sockets get address-bind only.
  - **Design decision (user-corrected model)**: vantage is a property of the *test run*, NOT the circuit — Pulsar is a measurement point; circuits don't terminate/originate here. A per-circuit source override stays possible later as an additive schema change if a reachability pattern (e.g. VLAN-only segments) demands it.
  - **Phase C — host network config mutation: deliberately DEFERRED.** Rationale: config editing is WARP Gateway's product identity (don't build it twice); a container can't own persistent host config (netplan/NM — live changes silently revert on reboot); remote-lockout risk needs Proxmox-style apply/revert engineering. If ever needed, the Pulsar-shaped version is scoped "test-lab addressing" (temporary secondary IP/VLAN with auto-revert).
...
- Deferred small items: f-string 3.12-only syntax portability (`node/main.py` MTR stream area); packet-capture interface dropdown is hardcoded (fixed by Phase A above).
